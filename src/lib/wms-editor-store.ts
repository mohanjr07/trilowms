...
export const useEditorStore = create<EditorState>()(
  persist(
    ...
    },
    {
      ...
      // The default warehouse's rack layout + forklift paths are reference/fixture
      // data (generated by makeZone/computeAislePath), not something users edit —
      // so anyone who already has the old, narrower rack spacing (and the
      // rack-clipping forklift paths that went with it) persisted from before this
      // fix keeps replaying it forever unless we backfill the shipped default
      // warehouse's zones + forklifts here too.
      merge: (persisted, current) => {
        ...
        const warehouses = (p.warehouses ?? current.warehouses).map((w) =>
          w.name === defaultWarehouse.name
            ? { ...w, zones: defaultWarehouse.zones, forklifts: defaultWarehouse.forklifts }
            : w,
        );
