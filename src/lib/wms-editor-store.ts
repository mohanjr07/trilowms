...
export const useEditorStore = create<EditorState>()(
  persist(
    ...
    },
    {
      ...
      // The default warehouse's rack layout, forklift paths, and dock spacing
      // are reference/fixture data (generated by makeZone/computeAislePath, or
      // fixed dock pitch), not something users edit — so anyone who already
      // has the old, narrower rack spacing/dock pitch (and the rack-clipping
      // forklift paths, or dock-truck-colliding dock pitch, that went with
      // it) persisted from before these fixes keeps replaying it forever
      // unless we backfill the shipped default warehouse's zones/forklifts/docks here too.
      merge: (persisted, current) => {
        ...
        const warehouses = (p.warehouses ?? current.warehouses).map((w) =>
          w.name === defaultWarehouse.name
            ? { ...w, zones: defaultWarehouse.zones, forklifts: defaultWarehouse.forklifts, docks: defaultWarehouse.docks }
            : w,
        );
